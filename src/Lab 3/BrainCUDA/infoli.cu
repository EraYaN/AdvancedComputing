...
int main(int argc, char *argv[]) {
	...
	checkCudaCall(cudaSetDevice(devID));
	...
	checkCudaCall(cudaMalloc((void**)&dev_iApp, simSteps * sizeof(double)));
	checkCudaCall(cudaMalloc((void**)&dev_cellStatePtr, IO_NETWORK_DIM1*IO_NETWORK_DIM2*PARAM_SIZE * sizeof(double)));
	checkCudaCall(cudaMalloc((void**)&dev_cellVDendPtr, IO_NETWORK_DIM1*IO_NETWORK_DIM2 * sizeof(double)));
	...
	t0 = now();
	checkCudaCall(cudaMemcpy(dev_iApp, iApp, simSteps * sizeof(double), cudaMemcpyHostToDevice));
	checkCudaCall(cudaMemcpy(dev_cellStatePtr, cellStatePtr, IO_NETWORK_DIM1*IO_NETWORK_DIM2*PARAM_SIZE * sizeof(double), cudaMemcpyHostToDevice));
	checkCudaCall(cudaMemcpy(dev_cellVDendPtr, cellVDendPtr, IO_NETWORK_DIM1*IO_NETWORK_DIM2 * sizeof(double), cudaMemcpyHostToDevice));
	checkCudaCall(cudaBindTexture2D(NULL, t_cellVDendPtr, dev_cellVDendPtr, desc, IO_NETWORK_DIM1, IO_NETWORK_DIM2, IO_NETWORK_DIM1 * sizeof(double)));
	t1 = now();
