                ...
                for data_size in data_sizes:
                    for variant in type['variants']:
                        ...
                        for n in range(0,max_n):
                            ...
                            result = subprocess.run(["../{0}{1}/{2}.exe".format(platform_paths[platform],config,type['name']),"-t {0}".format(threads),"-s {0}".format(data_size),"-v {0}".format(variant.value)],stdout=subprocess.PIPE,universal_newlines=True,cwd="../{0}{1}/".format(platform_paths[platform],config))
